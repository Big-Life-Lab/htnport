...
# glm
male_final_model <- config$models$male_final_model
female_final_model <- config$models$female_final_model
male_final_reduced_model <- config$models$male_final_reduced_model
female_final_reduced_model <- config$models$female_final_reduced_model
...
# 4. Validate with synthetic data before saving
cycle1 <- config$data$`cycle1-synth`
...
new_data <- dplyr::select(new_data, c("clc_age", "fmh_15", "diabx", "hwmdbmi"))

new_data$fmh_15[2] <- NA
new_data$diabx   <- ifelse(new_data$diabx == 1, 1,
                           ifelse(new_data$diabx == 2, 0, NA))
new_data$fmh_15  <- ifelse(new_data$fmh_15 == 1, 1,
                           ifelse(new_data$fmh_15 == 2, 0, NA))
