...
# Missing values in any raw variables (eg. blood creatine and ethnicity) and continuous derived variables (eg. glomerular filtration rate are properly recoded, but missing values in derived categorical variables (eg. having chronic kidney disease) are not recoded.
# Ex: In the synthetic cycle5, there are 2 observations with missing data needed to compute GFR and thus CKD status which is derived based on GFR (see kidney.R). Therefore, there should be 2 missing GFRs and subsequently 2 missing CKDs, but only missing data for GFR is shown but not CKD. Why is that?
create_descriptive_table( 
  table1_data,
  my_variables,
  my_variable_details,
