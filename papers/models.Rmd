...
calibration(male_predicted_probabilities, male_data_nofmh$highbp14090_adj)
calibration(female_predicted_probabilities, female_data_nofmh$highbp14090_adj)

# Fit reduced male and female models with unimputed family history and see effect
weighted_male_nofmh$degf <- 68
male_reduced_model_nofmh <- survey::svyglm(highbp14090_adj ~ rcs(clc_age, 4) + fmh_15 + rcs(clc_age, 4)*rcs(whr, 3) + rcs(clc_age, 4)*diabx, design = weighted_male_nofmh, family = quasibinomial())
...
# Fit reduced models with new family history predictor to see effect
weighted_male_nofmh$degf <- 68
male_reduced_model_nofmh <- survey::svyglm(highbp14090_adj ~ rcs(clc_age, 4) + fmh_15 + rcs(clc_age, 4)*rcs(whr, 3) + rcs(clc_age, 4)*diabx, design = weighted_male_nofmh, family = quasibinomial())
...
calibration(male_predicted_probabilities, unimputed_male_data$highbp14090_adj)
calibration(female_predicted_probabilities, unimputed_female_data$highbp14090_adj)

# Fit reduced male and female models with unimputed data and see effect
unimputed_weighted_male$degf <- 68
unimputed_male_reduced_model <- survey::svyglm(highbp14090_adj ~ rcs(clc_age, 4) + fmh_15 + rcs(clc_age, 4)*rcs(whr, 3) + rcs(clc_age, 4)*diabx, design = unimputed_weighted_male, family = quasibinomial())
