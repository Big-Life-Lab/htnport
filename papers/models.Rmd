...
### Log OR plots for non-linear predictors and interactions (full models only)
...
for (var in rcs_variables) {
  ...
  # Compute log(OR) and SE
  preds_male <- preds_male %>%
    mutate(
      logit = log(predicted / (1 - predicted)),
      se_logit = std.error / (predicted * (1 - predicted)),
      ref_pred = ref_row_m$predicted,
      ref_se = ref_row_m$std.error,
      ref_logit = log(ref_pred / (1 - ref_pred)),
      ref_se_logit = ref_se / (ref_pred * (1 - ref_pred)),
      log_or = logit - ref_logit,
      se_log_or = sqrt(se_logit^2 + ref_se_logit^2),
      log_or_low = log_or - 1.96 * se_log_or,
      log_or_high = log_or + 1.96 * se_log_or
    )
  ...
  preds_female <- preds_female %>%
    mutate(
      logit = log(predicted / (1 - predicted)),
      se_logit = std.error / (predicted * (1 - predicted)),
      ref_pred = ref_row_f$predicted,
      ref_se = ref_row_f$std.error,
      ref_logit = log(ref_pred / (1 - ref_pred)),
      ref_se_logit = ref_se / (ref_pred * (1 - ref_pred)),
      log_or = logit - ref_logit,
      se_log_or = sqrt(se_logit^2 + ref_se_logit^2),
  ...
  # Male plot
  print(ggplot(preds_male, aes(x = x, y = log_or)) +
    geom_line(color = "black", size = 1.2) +
    geom_ribbon(aes(ymin = log_or_low, ymax = log_or_high), fill = "grey", alpha = 0.3) +
    geom_hline(yintercept = 0, linetype = "dotted") +
    labs(title = paste0("Log(OR) by ", title_label, " in men"),
         x = x_label, y = "Log adjusted odds ratio") +
    theme_minimal())

  # Female plot
  print(ggplot(preds_female, aes(x = x, y = log_or)) +
    geom_line(color = "black", size = 1.2) +
    geom_ribbon(aes(ymin = log_or_low, ymax = log_or_high), fill = "grey", alpha = 0.3) +
    geom_hline(yintercept = 0, linetype = "dotted") +
    labs(title = paste0("Log(OR) by ", title_label, " in women"),
         x = x_label, y = "Log adjusted odds ratio") +
    theme_minimal())
}
...
# See effect on age interaction log(OR) plots
for (var in age_cat_int_variables) {
  print(plot_age_int_or(
