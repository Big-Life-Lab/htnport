...
#'
#' @description This function evaluates inclusive diabetes status based on three factors: `diab_m`, `CCC_51`, and `diab_drug`.
#'
...
#' @param diab_drug An integer indicating whether the respondent is on diabetes medication. 1 for "Yes", 0 for "No".
#'
#' @return An integer indicating the inclusive diabetes status:
#'         - 1 ("Yes") if any of `diab_m`, `CCC_51`, or `diab_drug` is 1.
#'         - 2 ("No") if all of `diab_m`, `CCC_51`, and `diab_drug` are 2.
...
#' determine_inclusive_diabetes(diab_m = 1, CCC_51 = 2, diab_drug = 2)
...
#' determine_inclusive_diabetes(diab_m = 2, CCC_51 = 2, diab_drug = 2)
#' # Output: 2 (Inclusive diabetes status is "No").
#'
#' # Example: Determine inclusive diabetes status when only one parameter is NA.
#' determine_inclusive_diabetes(diab_m = 2, CCC_51 = NA, diab_drug = 1)
#' # Output: 1 (Based on `diab_drug`, inclusive diabetes status is "Yes").
#'
#' # Example: Determine inclusive diabetes status when two parameters are NA.
#' determine_inclusive_diabetes(diab_m = NA, CCC_51 = NA, diab_drug = 2)
#' # Output: 2 (Based on `diab_drug`, inclusive diabetes status is "No").
#'
#' @export
determine_inclusive_diabetes <- function(diab_m, CCC_51, diab_drug) {
  
  diabX <- haven::tagged_na("b")  # Default value as tagged NA
  
  # Case 1: All are not NA
  if (!is.na(diab_m) && !is.na(CCC_51) && !is.na(diab_drug)) {
    if (diab_m == 1 || CCC_51 == 1 || diab_drug == 1) {
      diabX <- 1 # "Yes" if any of diab_m, CCC_51, or diab_drug is 1
    } else if (diab_m == 2 && CCC_51 == 2 && diab_drug == 0) {
      diabX <- 2 # "No" if all are 2
    }
  }
  
  # Case 2: All are NA
  else if (is.na(diab_m) && is.na(CCC_51) && is.na(diab_drug)) {
    diabX <- haven::tagged_na("b")
  }
  
  # Case 3: Two values are NA, check the remaining one
  else if (is.na(diab_m) && is.na(CCC_51)) {
    if (!is.na(diab_drug) && diab_drug == 1) {
      diabX <- 1 
    } else if (!is.na(diab_drug) && diab_drug == 0) {
      diabX <- 2  
    }
  } else if (is.na(diab_m) && is.na(diab_drug)) {
    if (!is.na(CCC_51) && CCC_51 == 1) {
      diabX <- 1 
    } else if (!is.na(CCC_51) && CCC_51 == 2) {
      diabX <- 2  
    }
  } else if (is.na(CCC_51) && is.na(diab_drug)) {
  ...
  # Case 4: Only one value is NA, check the other two
    ...
    if (CCC_51 == 1 || diab_drug == 1) {
      diabX <- 1 # "Yes" if any of the non-NA values is 1
    } else if (CCC_51 == 2 && diab_drug == 0) {
      diabX <- 2 # "No" if both non-NA values are 2
    }
  } else if (is.na(CCC_51)) {
    if (diab_m == 1 || diab_drug == 1) {
      diabX <- 1 # "Yes" if any of the non-NA values is 1
    } else if (diab_m == 2 && diab_drug == 0) {
      diabX <- 2 # "No" if both non-NA values are 2
    }
  } else if (is.na(diab_drug)) {
