...
### Table 2c - HTN based on medications only
...
# Define the logic using dplyr
htn_med_only_confirmed <- cycles1to6_data %>%
  # Step 1: Must be classified hypertensive
  dplyr::filter(highbp14090_adj == 1) %>%
  
  # Step 2: Must be on medication
  dplyr::filter(anymed2 == 1) %>%
  
  # Step 3: Must NOT meet BP thresholds (i.e., BP normal/low or missing)
  dplyr::filter(
  ...
  # Step 4: Stratify by sex
  dplyr::group_by(clc_sex) %>%
  dplyr::summarise(`HTN via meds only (confirmed HTN cases)` = dplyr::n()) %>%
  dplyr::ungroup()

flextable::flextable(htn_med_only_confirmed)
```