---
title: "Exporting HTNPoRT Full Models"
format: html
editor: visual
---
...
male_full_model <- survey::svyglm(
  ...
  design = weighted_male_c, family = quasibinomial())

weighted_female_c$degf <- 68
female_full_model <- survey::svyglm(
  highbp14090_adj ~ clc_age_rcs_1_C + clc_age_rcs_2_C + clc_age_rcs_3_C +
    married_2_C + married_3_C + edudr04_1_C + edudr04_2_C + working_2_C +
    gendmhi_1_C + gen_025_2_C + gen_045_2_C + fmh_15_1_C + hwmdbmi_rcs_1_C +
    hwmdbmi_rcs_2_C + low_drink_score1_2_C + low_drink_score1_3_C +
    ...
    clc_age_rcs_2_by_diabx_1_C + clc_age_rcs_3_by_diabx_1_C +
    clc_age_rcs_1_by_ckd_1_C + clc_age_rcs_2_by_ckd_1_C + clc_age_rcs_3_by_ckd_1_C,
  design = weighted_female_c, family = quasibinomial())

# 2. Create data summary (without individual values)
male_data_summary <- list(
  clc_age = list(mean = 46.14509835),
  married = list(mean1 = 0.085947731, mean2 = 0.248462597),
  edudr04 = list(mean1 = 0.217177205, mean2 = 0.125063569),
  working = list(mean = 0.258671725),
  gendmhi = list(mean = 0.066840869),
  gen_025 = list(mean = 0.228373362),
  gen_045 = list(mean = 0.349876734),
  fmh_15 = list(mean = 0.524803077),
  hwmdbmi = list(mean = 27.66777298),
  low_drink_score1 = list(mean1 = 0.777985051, mean2 = 0.045626893, mean3 = 0.072198781),
  minperweek = list(mean = 147.6496834),
  smoke = list(mean1 = 0.308696985, mean2 = 0.227724746),
  slp_11 = list(mean = 7.015035776),
  totalfv = list(mean = 3.280091672),
  diabx = list(mean = 0.104918358),
  ckd = list(mean = 0.044784256),
  outcome_prevalence = 0.25
)

female_data_summary <- list(
  clc_age = list(mean = 46.89545843),
  married = list(mean1 = 0.147107554, mean2 = 0.210100305),
  edudr04 = list(mean1 = 0.193357648, mean2 = 0.099653209),
  working = list(mean = 0.353413901),
  gendmhi = list(mean = 0.088527417),
  gen_025 = list(mean = 0.257966189),
  gen_045 = list(mean = 0.342661718),
  fmh_15 = list(mean = 0.573552958),
  hwmdbmi = list(mean = 27.0963796),
  low_drink_score1 = list(mean1 = 0.753166452, mean2 = 0.03853446, mean3 = 0.03202675),
  minperweek = list(mean = 126.2691998),
  smoke = list(mean1 = 0.272198263, mean2 = 0.165152547),
  slp_11 = list(mean = 7.176308826),
  totalfv = list(mean = 3.700420139),
  diabx = list(mean = 0.073367263),
  ckd = list(mean = 0.057604582),
  outcome_prevalence = 0.22
)

# 3. Create safe export
male_safe_full_model <- create_safe_export(male_full_model, male_data_summary)
female_safe_full_model <- create_safe_export(female_full_model, female_data_summary)

# 4. Save model objects
saveRDS(male_safe_full_model, file = here::here("output", "full-models", "model-object", "male_full_model.rds"))
saveRDS(female_safe_full_model, file = here::here("output", "full-models", "model-object", "female_full_model.rds"))
```
...
male_safe_full_model <- readRDS(here::here("output", "full-models", "model-object", "male_full_model.rds"))
female_safe_full_model <- readRDS(here::here("output", "full-models", "model-object", "female_full_model.rds"))

# 6. Validate cleanliness of model objects
testthat::test_that("Model objects are clean", {
  testthat::expect_true(validate_export(male_safe_full_model))
  testthat::expect_true(validate_export(female_safe_full_model))
})

# 7. Load validation data
male_validation_data <- read.csv(here::here("output", "full-models", "validation-data", "HTNPoRT-full-male-validation-data.csv"))
female_validation_data  <- read.csv(here::here("output", "full-models", "validation-data", "HTNPoRT-full-female-validation-data.csv"))

# 8. Make predictions using validation data
new_male_predictions <- predict(male_safe_full_model, newdata = male_validation_data , type = "response")
new_female_predictions <- predict(female_safe_full_model, newdata = female_validation_data, type = "response")
```
