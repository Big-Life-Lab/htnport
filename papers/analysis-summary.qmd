    ...
    ~ .x %>%
      dplyr::mutate(
        label = dplyr::case_when(
          ...
          variable == "Alcohol consumption level" & label == "2" ~ "Former or light drinker",
...
# Fit full male and female models with BMI only and see effect
weighted_male$degf <- 68
male_model_bmi <- survey::svyglm(highbp14090_adj ~ rcs(clc_age, 4) + married + edudr04 + working + gendmhi + gen_025 + gen_045 + fmh_15 + rcs(hwmdbmi, 3) + low_drink_score1 + rcs(minperweek, 3) + smoke + slp_11 + totalfv + diabx + ckd + rcs(clc_age, 4)*gen_045 + rcs(clc_age, 4)*rcs(hwmdbmi, 3) + rcs(clc_age, 4)*rcs(minperweek, 3) + rcs(clc_age, 4)*smoke + rcs(clc_age, 4)*slp_11 + rcs(clc_age, 4)*diabx + rcs(clc_age, 4)*ckd, design = weighted_male, family = quasibinomial())

weighted_female$degf <- 68
female_model_bmi <- survey::svyglm(highbp14090_adj ~ rcs(clc_age, 4) + married + edudr04 + working + gendmhi + gen_025 + gen_045 + fmh_15 + rcs(hwmdbmi, 3) + low_drink_score1 + rcs(minperweek, 3) + smoke + slp_11 + totalfv + diabx + ckd + rcs(clc_age, 4)*gen_045 + rcs(clc_age, 4)*rcs(hwmdbmi, 3) + rcs(clc_age, 4)*rcs(minperweek, 3) + rcs(clc_age, 4)*smoke + rcs(clc_age, 4)*slp_11 + rcs(clc_age, 4)*diabx + rcs(clc_age, 4)*ckd, design = weighted_female, family = quasibinomial())

male_predicted_probabilities <- generate_predicted_probabilities(male_model_bmi)
female_predicted_probabilities <- generate_predicted_probabilities(female_model_bmi)

calibration(male_predicted_probabilities, model.frame(male_model_bmi)$highbp14090_adj)
calibration(female_predicted_probabilities, model.frame(female_model_bmi)$highbp14090_adj)
