...
extract_centering_info <- function(model, original_data_summary) {
  ...
  predictors <- all.vars(formula(model))[-1]  # Exclude response
  
  for (pred in predictors) {
    if (pred %in% names(original_data_summary)) {
      centering_info[[pred]] <- list(
        center = original_data_summary[[pred]]$mean
      )
    }
  }
...
extract_spline_info <- function(model) {
  ...
  # Get the model matrix to access attributes
  mm <- model.matrix(model)
  
  # Look for spline-related attributes
  attrs <- attributes(mm)
  
  # Common spline packages store knots differently
  # For rms::rcs
  if ("parms" %in% names(attrs)) {
    spline_info$parms <- attrs$parms
  }
  
  # For splines::ns
  if ("knots" %in% names(attrs)) {
    spline_info$knots <- attrs$knots
  }
  
  # Extract from individual columns
  for (col in colnames(mm)) {
    col_attrs <- attributes(mm[,col])
    if (!is.null(col_attrs$knots)) {
      spline_info[[col]] <- list(knots = col_attrs$knots)
    }
  }
...
```{r}
extract_factor_info <- function(model) {
  factor_info <- list()
...
```{r}
create_safe_export <- function(svyglm_model, data_summary = NULL) {
  ...
  safe_export <- list(
    ...
    factor_info = extract_factor_info(svyglm_model),
    
    # For recalibration
    original_prevalence = if (svyglm_model$family$family == "binomial") {
...
```{r}
predict.safe_svyglm <- function(object, newdata, type = "link", se.fit = FALSE) {
  
  # Apply centering if available
  if (!is.null(object$centering)) {
    for (var in names(object$centering)) {
      if (var %in% names(newdata)) {
        newdata[[var]] <- (newdata[[var]] - object$centering[[var]]$center)
      }
    }
  }
  ...
  # Create model matrix
  X <- model.matrix(rhs_formula, new_data)

  # Predict using the stripped safe model
  eta <- as.vector(X %*% object$coefficients)

  # If response type is needed (e.g., for logistic models)
  predicted_probs <- object$family$linkinv(eta)
...
male_data_summary <- list(
  ...
  low_drink_score2 = list(mean = 0.778),
  low_drink_score3 = list(mean = 0.0456),
  low_drink_score4 = list(mean = 0.0722),
...
female_data_summary <- list(
  ...
  low_drink_score2 = list(mean = 0.7532),
  low_drink_score3 = list(mean = 0.0385),
  low_drink_score4 = list(mean = 0.032),
...
cycle1 <- data.frame(
  clc_age    = sample(20:79, n, replace = TRUE),
  fmh_15     = sample(1:2, n, replace = TRUE),
  lab_hba1   = runif(n, min = 0.041, max = 0.130),
  ccc_51     = sample(1:2, n, replace = TRUE),
  diab_drug2 = sample(c(0, 1), n, replace = TRUE),
  hwmdbmi    = runif(n, min = 13.89, max = 49)
)

cycle1$hwmdbmi[1] <- 99.96  # BMI way above expected
cycle1$fmh_15[2] <- 8       # Invalid family history code

new_data <- recodeflow::rec_with_table(cycle1, c("clc_age", "fmh_15", "lab_hba1", "ccc_51", "diab_drug2", "diab_m", "diabx", "hwmdbmi"), variable_details = my_variable_details)

new_data$fmh_15[2] <- NA
new_data$diabx   <- ifelse(new_data$diabx == 2, 1,
                           ifelse(new_data$diabx == 1, 0, NA))
new_data$fmh_15  <- ifelse(new_data$fmh_15 == 2, 1,
                           ifelse(new_data$fmh_15 == 1, 0, NA))

new_data$diabx   <- factor(new_data$diabx, levels = c(0, 1))
new_data$fmh_15  <- factor(new_data$fmh_15, levels = c(0, 1))

validate_export(male_reduced_model, male_safe_reduced_model, new_data)
validate_export(female_reduced_model, female_safe_reduced_model, new_data)
...
# 7. Make predictions
new_male_predictions <- predict(male_safe_reduced_model, new_data, type = "response")
new_female_predictions <- predict(female_safe_reduced_model, new_data, type = "response")
