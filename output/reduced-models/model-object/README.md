...
The `male_reduced_model.rds` and `female_reduced_model.rds` objects are `safe_svyglm` lists with the following components:
...
The `male_reduced_model.rds` and `female_reduced_model.rds` objects can be loaded into R using `readRDS()` and used with a custom `predict.safe_svyglm` function to make predictions on new data. See the `model-export.qmd` file for an example of how to create and use the objects.
