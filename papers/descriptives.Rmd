...
table1a_data <- get_descriptive_data(
  ...
  c("clc_sex", recodeflow:::select_vars_by_role(
    c("predictor"),
    my_variables
  )),
  # Sets the stratifier
...
weighted_imputed_data <- survey::svydesign(
  id = ~1,
  weights = ~wgt_full,
  data = dplyr::select(imputed_cycles1to6_data, c('Hypertension (Adjusted BPs)', 'Hypertension (Unadjusted BPs)', 'Controlled Hypertension (Adjusted BPs)', 'Controlled Hypertension (Unadjusted BPs)', clc_sex, wgt_full))
)
