...
level_labels <- list(
  ...
  "diabx" = c("1" = "Yes", "0" = "No"),
  "ckd" = c("1" = "Yes", "0" = "No"),
  "low_drink_score1" = c("1" = "Never drank", "2" = "Former or light drinker", "3" = "Moderate drinker", "4" = "Heavy drinker"),
...
### Sensitivity analyses (WHR but not BMI)

```{r, warning = FALSE, message = FALSE}
# Reobtain centered weighted objects
male_data <- dplyr::filter(imputed_cycles1to6_data, clc_sex == 1)
female_data <- dplyr::filter(imputed_cycles1to6_data, clc_sex == 2)
...
# Fit full male and female models with WHR only and see effect
weighted_male$degf <- 68
...
weighted_female$degf <- 68
female_model_whr <- survey::svyglm(highbp14090_adj ~ rcs(clc_age, 4) + married + edudr04 + working + gendmhi + gen_025 + gen_045 + fmh_15 + rcs(whr, 3) + low_drink_score1 + rcs(minperweek, 3) + smoke + slp_11 + totalfv + diabx + ckd + rcs(clc_age, 4)*gen_045 + rcs(clc_age, 4)*rcs(whr, 3) + rcs(clc_age, 4)*rcs(minperweek, 3) + rcs(clc_age, 4)*smoke + rcs(clc_age, 4)*slp_11 + rcs(clc_age, 4)*diabx + rcs(clc_age, 4)*ckd, design = weighted_female, family = quasibinomial())
...
write.csv(male_glm_betas, file = "P:/10619/Dropbox/htnport/vignettes/Model Presentation Output/Male Full GLM Model Betas.csv")
write.csv(female_glm_betas, file = "P:/10619/Dropbox/htnport/vignettes/Model Presentation Output/Female Full GLM Model Betas.csv")
...
get_weighted_quantiles("hwmdbmi", weighted_male, c(0.25, 0.50, 0.75))
get_weighted_quantiles("minperweek", weighted_male, c(0.25, 0.50, 0.75))
get_weighted_quantiles("slp_11", weighted_male, c(0.25, 0.50, 0.75))

get_weighted_quantiles("hwmdbmi", weighted_female, c(0.25, 0.50, 0.75))
get_weighted_quantiles("minperweek", weighted_female, c(0.25, 0.50, 0.75))
get_weighted_quantiles("slp_11", weighted_female, c(0.25, 0.50, 0.75))
```