...
#   agegroup4 = sample(c("xx", "xx", "xx", "xx"), 1000, replace = TRUE), # FIXED ROW COUNT
...
# male_data <- male_data %>%
#   dplyr::mutate(wgt_full = wgt_full / sum(wgt_full))
...
male_full_betas <- extract_beta_coefs(male_final_model)
female_full_betas <- extract_beta_coefs(female_final_model)
...
observation <- data.frame(
  ...
  hwmdbmi = 24,         # BMI = 24; Healthy but close to overweight 
  whr = 0.5,            # WHR = 0.5; Borderline of increased risk
  low_drink_score1 = 2, # Former drinker
  minperweek = 150,     # Exercises 150 min per week (barely meets criteria)
  smoke = 0,            # Never smoked  
  slp_11 = 6,           # Sleeps 6 hours per night
  totalfv = 4,          # Eats fruits and veggies 4 times per day (does not meet criteria)
...
# Fit final full male model with glm to avoid errors
male_reduced_model <- glm(highbp14090_adj ~ rcs(clc_age, 4) + fmh_15 + rcs(clc_age, 4)*rcs(whr, 3) + rcs(clc_age, 4)*diabx, family = quasibinomial(), data = male_data, weights = wgt_full)
