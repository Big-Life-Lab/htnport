...
### Table 1b - Weighted unimputed data
...
### Table 1c - Weighted imputed data
...
### Table 2b - HTN based on medications only
...
recode_na <- function(column) {
  # Convert the column to character if it's a factor
  if (is.factor(column)) {
    column <- as.character(column)
  }
  # Replace "NA(c)" with "NA(b)"
  column[column == "NA(c)"] <- NA
  return(column)
}

cycles1to6_data$ckd <- recode_na(cycles1to6_data$ckd)
cycles1to6_data$diabx <- recode_na(cycles1to6_data$diabx)

# Create a binary variable for the "medication only" subgroup in the full dataset
cycles1to6_data <- cycles1to6_data %>%
  dplyr::mutate(meds_only_htn = dplyr::case_when(
    highbp14090_adj == 1 &
    anymed2 == 1 &
    (is.na(sbp_adj) | is.na(dbp_adj) |
      (((!is.na(diabx) & diabx == 1) | (!is.na(ckd) & ckd == 1)) & sbp_adj < 130 & dbp_adj < 80) |
      ((is.na(diabx) | diabx != 1) & (is.na(ckd) | ckd != 1) & sbp_adj < 140 & dbp_adj < 90))
    ~ 1,
    TRUE ~ 0
  ))

# Create a survey design object using the full dataset
weighted_data <- srvyr::as_survey(cycles1to6_data, weights = wgt_full)

# Calculate the summary statistics
htn_med_only_summary <- weighted_data %>%
  dplyr::group_by(clc_sex) %>%
  dplyr::summarise(
    # Number of people with HTN based on meds only
    n_meds_only = unweighted(sum(meds_only_htn)),
    # Total number of people in the stratum (for unweighted %)
    n_total = unweighted(n()),
    # Weighted percentage of HTN based on meds only, out of the whole population
    weighted_percent = survey_mean(meds_only_htn) * 100
  ) %>%
  # Calculate unweighted percentage
  dplyr::mutate(
    unweighted_percent = (n_meds_only / n_total) * 100
  ) %>%
  # Select and rename the final columns
  dplyr::select(
    `Sex` = clc_sex,
    `HTN via meds only` = n_meds_only,
    `Unweighted %` = unweighted_percent,
    `Weighted %` = weighted_percent
  )

# Display the table
flextable::flextable(htn_med_only_summary)

# Table breaking down meds only HTN respondents by BP
table2c_data <- cycles1to6_data %>%
  # Step 1: Must be classified hypertensive and be on medication
  filter(highbp14090_adj == 1 & anymed2 == 1) %>%
  
  # Step 2: Must also have BP below diagnostic thresholds (or NA)
  filter(
    is.na(sbp_adj) | is.na(dbp_adj) |
      (((!is.na(diabx) & diabx == 1) | (!is.na(ckd) & ckd == 1)) & sbp_adj < 130 & dbp_adj < 80) |
      ((is.na(diabx) | diabx != 1) & (is.na(ckd) | ckd != 1) & sbp_adj < 140 & dbp_adj < 90)
  ...
  group_by(clc_sex) %>%
  summarise(
    # Counts for people WITHOUT diabetes or CKD
    normal_bps = sum(
      ((is.na(diabx) | diabx == 2) & (is.na(ckd) | ckd == 2)) & 
      (sbp_adj < 140) & (dbp_adj < 90),
      na.rm = TRUE
    ),

    
    # Counts for people WITH diabetes or CKD
    diab_or_ckd_normal_bps = sum(
      ((!is.na(diabx) & diabx == 1) | (!is.na(ckd) & ckd == 1)) & 
      (sbp_adj < 130) & (dbp_adj < 80),
      na.rm = TRUE
    ),
    
    # Counts for people WITH NA BPs
    na_bps = sum(
      (is.na(sbp_adj)) | (is.na(dbp_adj)),
      na.rm = TRUE
    ),
...
flextable::flextable(table2c_data)
```

### Table 2c - Weighted crude ORs and CIs for candidate predictors
